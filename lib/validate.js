...
module.exports = async function(spec, data, opt = {}) {
  ...
  for (const key in dotspec) {
    let path = key.split('.')
    if (!validators.includes(path.slice(-1)[0])) {
      path.push('eq')
    }
    let type = path.slice(-1)[0]
    let field = path.slice(0, -1).join('.')

    let want = _.get(dotspec, key)
    let got = _.get(data, field)
    ...
    if (_.isUndefined(got)) {
      ...
      type == 'minlength' && (!_.isString(got) || want > got.length) ||
      type == 'maxlength' && (!_.isString(got) || want < got.length) ||
